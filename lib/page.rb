...
#     if anchor_match = link_href.match( /^(#.*)/ )
#       # keep in page anchors on page
#       link.set_attribute :href, "#{@browser.state.current_url}#{anchor_match[1]}"
#     elsif ! link_to_file?( link_href )
#       begin
#         link_href = "#{@browser.base_href}#{link_href}" if URI.parse(link_href.to_s).relative?
#         link.set_attribute(
#           :href, 
#           browser_url( 
#             :t => URI.escape(link_href), 
#             :v => URI.escape(@browser.state.current_vertex.url), 
...
#     inputs = form.search( '//input' )
#     inputs.first.before( "<input type=hidden name='t' value='#{form_action}' />" )
#     inputs.first.before( "<input type=hidden name='v' value='#{URI.escape(@browser.state.current_vertex.url)}' />" )
#     inputs.first.before( "<input type=hidden name='n' value='<button>' />" )
#     
...
#   @document.search( '//head' ) do |head|
#     head_elements = head.search('/')
#     head_elements.first.before( "<base href='#{@browser.base_href}' />" )
#   end
# 
